#' @import dplyr
#' @export
dplyr::'%>%'

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @param y PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname trapez
#' @export
trapez = function(x,y){
	nx = length(x)
	0.5*sum((y[-1]+y[-nx])*(x[-1]-x[-nx]))
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @param y PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname pkpar
#' @export
pkpar = function(x,y){
	nx = length(x)
	auc = sum((x[-1]-x[-nx])*(y[-1]+y[-nx]),na.rm=T)/2
	nmax = order(y,na.last=F)[nx]
	cmax = y[nmax]
	tmax = x[nmax]
	c(auc=auc,cmax=cmax,tmax=tmax)
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname frac
#' @export
frac = function(x) sum(x)/length(x)

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @param oldval PARAM_DESCRIPTION
#' @param newval PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname swap
#' @export
swap = function(x, oldval, newval)
{
	if(length(oldval) != length(newval))
		stop("length(oldval)!=length(newval)")
	x1 = match(x, oldval, 0)
	x[as.logical(x1)] = newval[x1]
	x
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param y PARAM_DESCRIPTION
#' @param mu PARAM_DESCRIPTION
#' @param sigma2 PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname mvdnorm
#' @export
mvdnorm = function(y,mu,sigma2){
# bivariate norma density
	d = length(mu)
	det.sigma2 = det(sigma2)
	inv.sigma2 = solve(sigma2)
	exp(-t(y-mu)%*%inv.sigma2%*%(y-mu)/2)/sqrt(((2*pi)^d)*det.sigma2)
}

#' @title generalized logit and inverse logit functions
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @param lower PARAM_DESCRIPTION, Default: 0
#' @param upper PARAM_DESCRIPTION, Default: 1
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname logit
#' @export
logit.inv = function(x, lower = 0, upper = 1)
{
	x1 <- exp(x)
	ifelse(is.infinite(x1),upper,lower + ((upper - lower) * x1)/(1 + x1))
}

#' @rdname logit
#' @export
logit = function(x, lower = 0, upper = 1)
{
	x <- (x - lower)/(upper - lower)
	log(x/(1 - x))
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname factor2char.data.frame
#' @export
factor2char.data.frame = function(x){
	for(i in 1:ncol(x)){
		if(class(x[[i]])=="factor") x[[i]] = as.character(x[[i]])
	}
	x
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname strip.lead.blanks
#' @export
strip.lead.blanks = function(x){
	w = regexpr(" +",x)
	as.character(ifelse(w==1,substring(x,attr(w,"match.length")+1),x))
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param a PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname symmat
#' @export
symmat = function(a){
# generate symmetric matrix from its lower triangle given as
# (a[1,1],a[2,1],a[2,2],a[3,1],a[3,2],a[3,3],...)
	n = (-1+sqrt(1+8*length(a)))/2
	x = matrix(rep(0,n*n),ncol=n)
	x[t(lower.tri(x,T))] = a
	y = x + t(t(lower.tri(x))*x)
	y
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @param label PARAM_DESCRIPTION, Default: NULL
#' @param main PARAM_DESCRIPTION, Default: NULL
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @seealso
#'  \code{\link[lattice]{histogram}},\code{\link[lattice]{qqmath}},\code{\link[lattice]{panel.qqmath}},\code{\link[lattice]{panel.qqmathline}}
#' @rdname qqhist
#' @export
#' @importFrom lattice histogram qqmath panel.qqmath panel.qqmathline prepanel.qqmathline
qqhist <- function(x, label = NULL, main=NULL)
{
# plot histogram and quantile-normal quantile plot for vector x

	plot1 <- lattice::histogram(~x,
		ylab=list(cex=1.2),	xlab=list(label=label,cex=1.2),
		scales=list(cex=1.2))
	plot2 <- lattice::qqmath(~x,prepanel = lattice::prepanel.qqmathline,
		panel=function(x){
		  lattice::panel.qqmath(x,cex=1.2,pch=16)
		  lattice::panel.qqmathline(x,distribution=qnorm,col=3,lwd=3)
		},
		ylab=list(label=label,cex=1.2),
		xlab=list(label="quantiles of standard normal",cex=1.2),
		scales=list(cex=1.2),ylim=range(x))
	list(plot1=plot1,plot2=plot2)
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param p PARAM_DESCRIPTION
#' @param mean PARAM_DESCRIPTION, Default: 0
#' @param sd PARAM_DESCRIPTION, Default: 1
#' @param lower PARAM_DESCRIPTION, Default: -Inf
#' @param upper PARAM_DESCRIPTION, Default: Inf
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @seealso
#'  \code{\link[stats]{qnorm}},\code{\link[stats]{pnorm}}
#' @rdname qnorm.trunc
#' @export
#' @importFrom stats qnorm pnorm
qnorm.trunc = function(p,mean=0,sd=1,lower=-Inf,upper=Inf)
	stats::qnorm(p*stats::pnorm(upper,mean,sd)+(1-p)*stats::pnorm(lower,mean,sd),mean,sd)

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param n PARAM_DESCRIPTION
#' @param mean PARAM_DESCRIPTION, Default: 0
#' @param sd PARAM_DESCRIPTION, Default: 1
#' @param lower PARAM_DESCRIPTION, Default: -Inf
#' @param upper PARAM_DESCRIPTION, Default: Inf
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname rnorm.trunc
#' @export
#' @importFrom stats runif
rnorm.trunc = function(n,mean=0,sd=1,lower=-Inf,upper=Inf)
	qnorm.trunc(stats::runif(n),mean,sd,lower,upper)



#' @rdname panel.ci
#' @export
prepanel.ci <- function(y, ly, uy, subscripts, ...)
{
    y <- as.numeric(y)
    ly <- as.numeric(ly[subscripts])
    uy <- as.numeric(uy[subscripts])
    list(ylim = range(y, uy, ly, finite = TRUE))
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param x PARAM_DESCRIPTION
#' @param y PARAM_DESCRIPTION
#' @param ly PARAM_DESCRIPTION
#' @param uy PARAM_DESCRIPTION
#' @param subscripts PARAM_DESCRIPTION
#' @param pch PARAM_DESCRIPTION, Default: 16
#' @param col.line PARAM_DESCRIPTION, Default: 'black'
#' @param ... PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @seealso
#'  \code{\link[lattice]{panel.arrows}},\code{\link[lattice]{panel.xyplot}}
#' @rdname panel.ci
#' @export
#' @importFrom lattice panel.arrows panel.xyplot
panel.ci <- function(x, y, ly, uy, subscripts, pch = 16, col.line =
'black', ...)
{
    x <- as.numeric(x)
    y <- as.numeric(y)
    ly <- as.numeric(ly[subscripts])
    uy <- as.numeric(uy[subscripts])
    lattice::panel.arrows(x, ly, x, uy, col = col.line,
                 length = 0.25, unit = "native",
                 angle = 90, code = 3)
    lattice::panel.xyplot(x, y, pch = pch, col.line = col.line, ...)
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param data PARAM_DESCRIPTION
#' @param x PARAM_DESCRIPTION, Default: 'x'
#' @param treatment PARAM_DESCRIPTION, Default: 'treatment'
#' @param type PARAM_DESCRIPTION, Default: 'type'
#' @param xlab PARAM_DESCRIPTION, Default: 'x'
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @seealso
#'  \code{\link[lattice]{panel.xyplot}},\code{\link[lattice]{panel.segments}}
#' @rdname mbmaPpcPlot
#' @export
#' @importFrom lattice panel.xyplot panel.segments xyplot
mbmaPpcPlot <- function(data, x = "x", treatment = "treatment", type = "type", xlab = "x"){

  names(data)[names(data) == x] <- "x"
  names(data)[names(data) == treatment] <- "treatment"
  names(data)[names(data) == type] <- "type"
  xobs <- data[data$type == "observed",]

  ## Reorder assuming that the predicted values are in the same order as the observed values.
  xOrder <- order(xobs$treatment, xobs$x)
  xOrder <- rep(xOrder, 4) + rep(0:3, ea = length(xOrder)) * length(xOrder)
  data <- data[xOrder,]
  xobs <- data[data$type == "observed",]

  ## Calculate number of treatments and number of arms of each treatment
  treats <- unique(xobs$treatment)
  nTreat <- length(treats)
  nPerTreat <- sapply(treats, function(treat, data)
                      sum(data$treatment == treat),
                      data = xobs)

  ## Calculate number of equally spaced positions on the y axis
  ny <- sum(nPerTreat) + nTreat - 1
  dy <- 1 / (ny - 1)
  y <- (1:ny) * dy

  ## Drop y positions between treatments to create gaps
  iSkip <- cumsum(nPerTreat[-nTreat]) + 1:(nTreat - 1)
  y <- y[-iSkip]

  ## Calculate label locations
  iStart <- c(1, 1 + cumsum(nPerTreat[-nTreat]))
  iEnd <- cumsum(nPerTreat)
  yLab <- (y[iStart] + y[iEnd]) / 2

  data$y <- rep(y, 4)
  lattice::xyplot(y ~ x, data, groups = type, panel =
         function(x, y, subscripts, groups, ...){
           iObs <- (groups[subscripts] == "observed")
           iMed <- (groups[subscripts] == "median")
           i5 <- (groups[subscripts] == "5%ile")
           i95 <- (groups[subscripts] == "95%ile")
           lattice::panel.xyplot(x[iObs], y[iObs], pch = 19, col = "black", ...)
           lattice::panel.segments(x[iMed], y[iMed] - dy/2, x[iMed], y[iMed] + dy/2,
                          col = "red", lwd = 2, ...)
           lattice::panel.segments(x[i5], y[iMed], x[i95], y[iMed],
                          col = "blue", lwd = 2, ...)
         },
         scales = list(cex = 1.2, y = list(at = yLab, labels = treats)),
         xlab = xlab, ylab = "")
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param n PARAM_DESCRIPTION
#' @param mean PARAM_DESCRIPTION
#' @param sd PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @seealso
#'  \code{\link[stats]{rbeta}}
#' @rdname rbeta2
#' @export
#' @importFrom stats rbeta
rbeta2 <- function(n, mean, sd){
    v <- sd^2
    alpha <- (mean * (1 - mean) / v - 1) * mean
    beta <- (1 / mean - 1) * alpha
    if(alpha <= 0 | beta <= 0) stop("ERROR in rbeta2: alpha <= 0 | beta <= 0")
    stats::rbeta(n, alpha, beta)
}

#' @title FUNCTION_TITLE
#' @description FUNCTION_DESCRIPTION
#' @param n PARAM_DESCRIPTION
#' @param mean PARAM_DESCRIPTION
#' @param sd PARAM_DESCRIPTION
#' @param lower PARAM_DESCRIPTION
#' @param upper PARAM_DESCRIPTION
#' @return OUTPUT_DESCRIPTION
#' @details DETAILS
#' @examples
#' \dontrun{
#' if(interactive()){
#'  #EXAMPLE1
#'  }
#' }
#' @rdname rbeta2Scaled
#' @export
rbeta2Scaled <- function(n, mean, sd, lower, upper){
    dx <- upper - lower
    m <- (mean - lower) / dx
    s <- sd / dx
    if(m < 0 | m > 1) stop("ERROR in rbeta2Scaled: m < 0 | m > 1")
    rbeta2(n, m, s) * dx + lower
}
